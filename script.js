...
const AIRPORTS = [
    ...
    { ident: 'LFPG', name: 'Paris Charles de Gaulle Airport', lat: 49.0097, lon: 2.5479 },
    { ident: 'EHAM', name: 'Amsterdam Schiphol Airport', lat: 52.3086, lon: 4.7639 },
    ...
    { ident: 'LEBL', name: 'Barcelona El Prat Airport', lat: 41.2971, lon: 2.0785 },
    { ident: 'EDDM', name: 'Munich Airport', lat: 48.3538, lon: 11.7861 },
    { ident: 'LOWW', name: 'Vienna International Airport', lat: 48.1103, lon: 16.5697 },
    { ident: 'LSZH', name: 'Zurich Airport', lat: 47.4647, lon: 8.5492 },
    { ident: 'EKCH', name: 'Copenhagen Airport', lat: 55.6180, lon: 12.6508 },
    { ident: 'ENGM', name: 'Oslo Gardermoen Airport', lat: 60.1939, lon: 11.1004 },
    { ident: 'ESSA', name: 'Stockholm Arlanda Airport', lat: 59.6519, lon: 17.9186 },
    ...
    { ident: 'LGAV', name: 'Athens International Airport', lat: 37.9364, lon: 23.9445 },
    { ident: 'EPWA', name: 'Warsaw Chopin Airport', lat: 52.1657, lon: 20.9671 },
    { ident: 'LKPR', name: 'Prague Václav Havel Airport', lat: 50.1008, lon: 14.2600 },
    { ident: 'LHBP', name: 'Budapest Ferenc Liszt Airport', lat: 47.4369, lon: 19.2556 },
    ...
    { ident: 'EGCC', name: 'Manchester Airport', lat: 53.3537, lon: -2.2750 },
    { ident: 'EDDL', name: 'Düsseldorf Airport', lat: 51.2895, lon: 6.7668 }
];

// Mission Templates
const MISSION_TEMPLATES = [
    { disaster: 'Flood', icon: '🌊', severityRange: [2, 4], peopleRange: [3, 6], reward: 10 },
    { disaster: 'Crash', icon: '💥', severityRange: [3, 5], peopleRange: [2, 5], reward: 15 },
    { disaster: 'Earthquake', icon: '🏚️', severityRange: [4, 6], peopleRange: [5, 9], reward: 25 },
    { disaster: 'Fire', icon: '🔥', severityRange: [2, 4], peopleRange: [2, 5], reward: 12 },
    { disaster: 'Storm', icon: '🌪️', severityRange: [3, 5], peopleRange: [3, 7], reward: 18 }
];
...
const EVENTS = [
    {
        name: 'Fuel Leaked',
        description: 'Sabotage or mechanical failure caused a massive leak while you were grounded. Significant fuel loss!',
        probability: 4,
        fuelEffect: -400,
        rescuedEffect: 0,
        reputationEffect: 0,
    ...
    },
    {
        ...
        description: 'A secondary terrorist attack hits the runway. Your immediate response is scrutinized, causing reputation loss.',
        probability: 3,
        fuelEffect: -50,
        rescuedEffect: 0,
        reputationEffect: -50,
    ...
    {
        name: 'Bomb Threat',
        description: 'Authorities ground all flights due to a bomb threat. You lose time finding an alternate runway, costing fuel.',
        probability: 2,
        fuelEffect: -200,
        rescuedEffect: 0,
        reputationEffect: -10,
    ...
    {
        name: 'Plane Damage',
        description: 'Critical structural damage is discovered on your plane. Continuing is impossible.',
        probability: 1,
        fuelEffect: 0,
        rescuedEffect: 0,
        reputationEffect: -50,
    ...
    {
        name: 'Media Coverage',
        description: 'Your heroic efforts were featured on the news! Public trust in your mission increases.',
        probability: 2,
        fuelEffect: 0,
        rescuedEffect: 0,
        reputationEffect: 25,
    ...
    },
    {
        ...
        description: 'Local volunteers helped rescue additional people while you were landing!',
        probability: 2,
        fuelEffect: 0,
        rescuedEffect: 2,
        reputationEffect: 10,
...
function createMissions(airports) {
    ...
    missionAirports.forEach(airport => {
        ...
        const mission = {
            ...
            peopleInDanger: randomInt(template.peopleRange[0], template.peopleRange[1]),
...
function setupEventListeners() {
    ...
    document.getElementById('accept-mission').addEventListener('click', attemptMission);
    document.getElementById('decline-mission').addEventListener('click', declineMission);
...
function startGame() {
    ...
    // Update UI
    document.getElementById('pilot-name-display').textContent = gameState.playerName;
    updateUI();
}
...
function updateUI() {
    ...
    document.getElementById('fuel-bar').style.width = `${Math.min(100, fuelPercent)}%`;
    document.getElementById('fuel-value').textContent = Math.max(0, gameState.fuel);
    ...
    document.getElementById('reputation-bar').style.width = `${Math.min(100, repPercent)}%`;
    document.getElementById('reputation-value').textContent = gameState.reputation;
    
    // Update rescued count
    document.getElementById('rescued-value').textContent = gameState.rescuedPeople;
    
    // Update missions count
    document.getElementById('missions-completed').textContent = gameState.completedMissions;
    document.getElementById('missions-total').textContent = CONFIG.missionsToAssign;
    
    // Update current location
    document.getElementById('current-airport-name').textContent = gameState.currentAirport.name;
    document.getElementById('current-airport-code').textContent = gameState.currentAirport.ident;
    
    // Update refuel button visibility
    const refuelBtn = document.getElementById('refuel-btn');
    if (gameState.reputation >= CONFIG.rechargeCost && gameState.fuel < CONFIG.maxFuel) {
...
function updateAirportsGrid() {
    ...
    AIRPORTS.forEach(airport => {
        ...
        card.className = `airport-card ${inRange ? '' : 'out-of-range'} ${hasMission ? 'has-mission' : ''}`;
        ...
        card.innerHTML = `
            ...
            ${hasMission ? '<div class="mission-indicator">🚨 MISSION AVAILABLE</div>' : ''}
        `;
...
function checkMissionAtAirport() {
    const mission = gameState.missions.find(
        m => m.airport.ident === gameState.currentAirport.ident && m.status === 'pending'
    );
...
function showMissionModal(mission) {
    ...
    document.getElementById('disaster-type').textContent = mission.disaster.toUpperCase();
    document.getElementById('people-trapped').textContent = mission.peopleInDanger;
    document.getElementById('severity-level').textContent = `${mission.severity}/10`;
    document.getElementById('mission-reward').textContent = `+${mission.reward} Trust`;
...
function attemptMission() {
    ...
    if (successRoll >= mission.severity) {
        ...
        showResultModal(
            ...
            [
                { label: 'People Rescued', value: `+${mission.peopleInDanger}`, positive: true },
...
function showEventModal(event) {
    ...
    effectsContainer.innerHTML = effects.length > 0 ? effects.map(e => `
        <div class="effect-item ${e.positive ? 'positive' : 'negative'}">
            <span>${e.label}:</span>
            <span>${e.value}</span>
        </div>
    `).join('') : '<p style="text-align: center; color: var(--text-muted);">No effects</p>';
...
function handleRefuel() {
    if (gameState.reputation >= CONFIG.rechargeCost) {
        ...
        gameState.fuel = Math.min(CONFIG.maxFuel, gameState.fuel + CONFIG.rechargeGain);
        updateUI();
...
function checkWinCondition() {
    const pendingMissions = gameState.missions.filter(m => m.status === 'pending').length;
...
function showVictory() {
    ...
    document.getElementById('victory-rescued').textContent = gameState.rescuedPeople;
    document.getElementById('victory-reputation').textContent = gameState.reputation;
...
function showGameOver(reason) {
    ...
    document.getElementById('final-rescued').textContent = gameState.rescuedPeople;
    document.getElementById('final-reputation').textContent = gameState.reputation;
    document.getElementById('final-missions').textContent = `${gameState.completedMissions}/${CONFIG.missionsToAssign}`;
