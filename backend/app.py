...
CONFIG = {
    ...
    'missions_available': 12,      # Total missions on map
    'missions_to_complete': 6,     # Missions needed to win
    'recharge_cost': 20,
    'recharge_gain': 1000,
    'max_fuel': 3500,
    'max_reputation': 200
...
def get_airports():
    """Get random European large airports"""
...
def create_player():
    ...
    """)
    start_airport = cursor.fetchone()['ident']
    ...
    # Get airports for missions (excluding start airport) - NOW 12+ MISSIONS
    cursor.execute("""
        SELECT ident, name, latitude_deg, longitude_deg FROM airport 
        WHERE continent = 'EU' AND type = 'large_airport' AND ident != %s
        ORDER BY RAND() LIMIT %s
    """, (start_airport, CONFIG['missions_available']))
    
    mission_airports = cursor.fetchall()
    
    # Create missions with varying difficulties
    missions = []
    for i, airport in enumerate(mission_airports):
        # Distribute difficulties - some easy, some medium, some hard
        if i < 4:
            # Easy missions (lower severity)
            ...
            # Harder missions (higher rewards)
    ...
    # Get start airport info
    cursor.execute("SELECT ident, name, latitude_deg, longitude_deg FROM airport WHERE ident = %s", (start_airport,))
...
def attempt_mission(player_id, mission_id):
    ...
    # Roll for success - slightly favor the player
    success_roll = random.randint(1, 10)
    # Add a small bonus to make it easier (player gets +1 to their roll)
...
def skip_mission(player_id, mission_id):
    """Skip a mission without attempting it (small reputation cost)"""
    cursor = db.get_cursor()
...
def get_nearby_airports(player_id):
    ...
    # Get all European airports except current
    cursor.execute("""
        SELECT ident, name, latitude_deg, longitude_deg
        FROM airport
        WHERE continent = 'EU' AND type = 'large_airport' AND ident != %s
    ...
    # Get missions for this player (pending only)
    cursor.execute("""
    ...
    # Sort by missions first, then by distance
